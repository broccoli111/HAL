#!/usr/bin/env node
/** Owner-only deactivation of one registered folder's derived pack; no source is read. */
...
function fail(message) {
  process.stderr.write(`HAL owner-folder deactivate: ${message}\n`);
  process.exit(2);
}
...
if (!registration || registration.status !== "registered") {
  fail("registration is unavailable or revoked; no folder was read and no runtime was contacted.");
}
...
if (!active || active.packId !== packId) {
  fail("registered folder is not active; no state was changed.");
}
...
if (deactivation.result !== "succeeded") {
  fail(`deactivation failed closed: ${deactivation.resultReasonCode}.`);
}
process.stdout.write(`HAL owner-folder deactivated: ${registration.registrationId}\n`);
