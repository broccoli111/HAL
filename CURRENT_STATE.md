...
- A live one-question Canon-only smoke check through the existing restricted GX10 Hermes/Qwen route correctly answered that runtime memory is non-authoritative and not canonical HAL knowledge, citing the bounded Book II topic-index references. The response remains a runtime `unaccepted_runtime_claim`; the check added no capability, source, or persistent conversation state.
...
11. DR 0032 Option 2 is accepted for a bounded Owner-controlled local folder registry. HAL now has a fixed-policy registration/revocation contract, a HAL-only bounded source-snapshot collector, and a deterministic manifest-hashed per-folder pack artifact builder; deterministic tests reject relative/root paths, invalid Owner claims, unsupported file types, symlinks, and revoked registrations. No real folder is registered or available to the assistant yet. Next, add controlled artifact persistence/validation and activation/revocation conformance without exposing a path, handle, tool, or capability to the runtime.
...
**Owner-approved local folder registry design/conformance:** DR 0032 Option 2 authorizes a HAL-owned, explicit registration and revocation workflow for future local folders. The current implementation contains only the fixed-policy registration contract and conformance tests; it has not registered, read, or exposed another folder. Every later admitted folder must remain separately manifest-hashed, provenance-recorded, Owner-activated/revocable, non-canonical, and invisible to the runtime except through bounded HAL-rendered context.
