...
4. Completed: HAL-owned evidence now covers the validated zero-capability CLI, fixed repository-status/history compositions, and bounded success/denial behavior. Before any new capability proposal, obtain an Owner Decision on whether to authorize a non-synthetic controlled local-knowledge pilot.
...
**Owner Decision required — controlled non-synthetic knowledge:** The current M9 implementation expressly excludes private/user-document ingestion and arbitrary filesystem pack admission; its active proof pack is synthetic only. Making the local assistant informed by real HAL documentation would therefore require an Owner-authorized, source-scoped knowledge-pilot decision before implementation. The decision must identify the approved source set and ownership/classification, whether the material is canonical or non-canonical context, required manifest/provenance and review controls, and the prohibition on direct runtime filesystem access or runtime canonical-knowledge writes. Until then, the assistant remains limited to its general local-model responses, the approved synthetic pack, and the already fixed repository status/history compositions.
