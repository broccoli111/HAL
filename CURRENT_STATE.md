...
11. DR 0032 Option 2 is accepted for a bounded Owner-controlled local folder registry. HAL now has a fixed-policy registration/revocation contract and a HAL-only bounded source-snapshot collector; deterministic tests reject relative/root paths, invalid Owner claims, unsupported file types, symlinks, and revoked registrations. No real folder is registered or available to the assistant yet. Next, implement immutable per-folder pack generation and activation/revocation conformance without exposing a path, handle, tool, or capability to the runtime.
