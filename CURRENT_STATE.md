...
11. DR 0032 Option 2 is accepted for a bounded Owner-controlled local folder registry. The first HAL-owned registration/revocation policy contract and deterministic deny tests pass; no real folder is registered or available to the assistant yet. Next, implement immutable per-folder pack generation and activation/revocation conformance without exposing a path, handle, tool, or capability to the runtime.
