...
- A deterministic test-only full-Hermes CLI probe now fixes the local relay endpoint, uses a fixed synthetic prompt, and explicitly configures `platform_toolsets.cli: []`. Its GX10 invocation remained contained and was removed after execution, but the SSH channel did not retain its structured diagnostics; the full CLI compatibility finding remains unresolved and must not be represented as a pass.
...
4. Resolve the full Hermes CLI agent-loop compatibility finding under the same zero-capability, local-only containment posture before enabling any tool-capable runtime behavior.
