...
3. Use the Owner-authorized DR 0027 restricted transport for the first local assistant slice, then separately resolve the full Hermes CLI agent-loop compatibility finding before enabling any tool-capable runtime behavior.
4. Resolve the recorded DR 0026/DR 0027 prompt-scope conflict before extending the local assistant: DR 0026's design says "No general prompt support," while the DR 0027 restricted transport accepts bounded text questions. No broader runtime behavior may be enabled until the Owner either confirms that bounded local text questions are within the pilot or narrows the transport to synthetic prompts.
5. Use the Solo-Owner Assurance Profile only for eligible routine, reversible, non-production work; retain independent review for the high-risk threshold and production release. Exception 0024 applies only to the exact Book III amendment certification-control gate.
6. Do not broaden the completed DR 0012/0015/0019/0020 pilots or DR 0026 into an external provider, unapproved model, real capability, real resource, general mediation service, or production deployment.
...
GX10-1 evaluation evidence remains strictly bounded to the approved isolated `hal_eval` account, rootless container profile, static validation, safe no-provider smoke result, synthetic line-driver probes, the DR 0026 local-only Hermes/Qwen3 pilot, and the DR 0027 restricted stateless transport. The stateless result/evidence route is complete; full Hermes CLI-loop compatibility remains incomplete. The authorization does not permit production activation, credentials, governed resources, a general provider route, or tool-capable runtime behavior.

**Prompt-scope conflict requiring Owner clarification:** DR 0026's `Explicit Non-Goals` says "No general prompt support," but DR 0027's implemented restricted stateless route accepts a bounded text prompt and has been demonstrated with a normal arithmetic question. This does not affect the zero-capability, local-only containment controls or canonical-knowledge boundary, but it is a material scope ambiguity. Do not extend the assistant, enable the full Hermes CLI loop, or add any capability until the Owner explicitly selects either (a) a clarification permitting bounded local text-only questions under the existing controls, or (b) a rollback to fixed synthetic prompts only.
