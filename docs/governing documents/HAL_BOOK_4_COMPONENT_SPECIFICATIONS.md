...
| CMP-09-IF-03 | Command | Restrict Memory Association | Command `Restrict Memory Association` MUST carry identity, authority context where applicable, schema version, correlation, causation, time/freshness, provenance, and explicit success or denial semantics. It MUST use Book II's protected-deletion and tombstone model: it may remove or limit accessibility and derived associations, but MUST NOT erase, rewrite, or make unauditable historical experience or immutable Experience Ledger records. | Wire encoding, field schema, protocol, error code, timeout, retry, and compatibility contract |
...
| CMP-09-IF-09 | Event | Memory Association Restricted | Event `Memory Association Restricted` MUST carry identity, authority context where applicable, schema version, correlation, causation, time/freshness, provenance, and explicit success or denial semantics. It records the governed restriction or tombstone without asserting deletion of historical experience or immutable archive evidence. | Wire encoding, field schema, protocol, error code, timeout, retry, and compatibility contract |
...
Commands: Store Memory Association, Revise Association, Restrict Memory Association, Reconcile Conversation. Queries: Recall Memory, Trace Memory Source, Get Retention Status. Events: Memory Associated, Memory Association Restricted, Conversation Reconciled.

`Restrict Memory Association` and `Memory Association Restricted` are governed accessibility and association controls under Book II's protected-deletion and tombstone model. They MUST preserve historical experience, immutable transition evidence, and Experience Ledger custody; they MUST NOT rewrite history or implement literal forgetting of experience.
